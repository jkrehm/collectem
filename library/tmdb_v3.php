...
###############################################################################################################
    function  __construct($apikey) {
		...
		$this->setImageURL($conf)->setImageSizes($conf);
    }
	...
	*/
	public function searchMovie($movieTitle, $page=1)
	{
		...
		return $this->_call("search/movie", $movieTitle, array('page'=>$page));
	}
	...
	*/
	 ...
	 * @return string
	 */
	private function _call($action, $text, $arr_parms=null){
	// # http://api.themoviedb.org/3/movie/11?api_key=XXX
		$parms = '';
		if (is_array($arr_parms))
		{
			foreach ($arr_parms as $key => $value)
			{
				$parms .= "&$key=$value";
			}
			unset($value);
		...
		$url= TMDBv3::_API_URL_.$action."?api_key=".$this->getApikey()."&language=".$lang."&".$text.$parms;
	...
	* @return class
	*/
	public function setImageSizes($config) {
		unset($config['images']['base_url']);
		$this->_imgSizes = $config['images'];
		
		return $this;
	}

	/**
	 * Getter for the images sizes
	 *
	 * @return array
	 */
